...
dataset from the [Arctic Data Center](https://arcticdata.io/) data
repository.

As example, we are using permafrost data from the Polaris Project 2017:
...
inside your main directory; `Data_polaris` in this example. `metajam`
...
contains the data `polaris17_permafrost$data`, the general (summary)
metadata `polaris17_permafrost$summary_metadata` - such as title,
creators, dates, locations - and the attribute level metadata
information `polaris17_permafrost$attribute_metadata`, allowing user to
get more information, such as units and definitions of your attributes.
